...
//                    }
    ...
    public byte[] histeq(byte[] data, int width, int height){
        ...
        int[] hist_temp = new int[data.length];
        int[] cdf = new int[data.length];
        int[] norm_cdf = new int[data.length];
        int max = 0;
        int min = 0;
        boolean flag = true;
        ...
        for(int i=0;i<size;i++){
            hist_temp[0xff & (int)data[i]]++;        //Gets Histeq
        }
        for(int i=0; i<size; i++)       //CDF
        {
            if(i == 0)
            {
                cdf[i] = hist_temp[i];
        ...
        for (int i=0; i<size; i++)
        {
            norm_cdf[i] = (cdf[i] - min)*(255)/(size -1);
            histeqdata[i] = (byte)norm_cdf[0xff & (int)data[i]];
        }
    ...
    public int[] conv2(byte[] data, int width, int height, double kernel[][]){
        ...
        double[] convdata1 = new double[size];
            ...
            for (int j=0 ; j<width;j++)
            {
                for (int y=0; y<3; y++)
                {
                    for (int x=0; x<3; x++)
                    {
                        ...
                        {
                            convdata1[i * width + j] += (double) data[(i + y - 1) * width + (j + x - 1)] * kernel[x][y];
                        }
                    }
                }
                convdata1[i*width+j] = (convdata1[i*width+j] > 255) ? 255: convdata1[i*width+j];
                convdata1[i*width+j] = (convdata1[i*width+j] < 0) ? 0: convdata1[i*width+j];
                convdata[i * width + j] = (int) convdata1[i * width + j];
            }
        }
