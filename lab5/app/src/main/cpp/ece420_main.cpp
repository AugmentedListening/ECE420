...
bool lab5PitchShift(float *bufferIn) {
    ...
    if (periodLen > 0) {
        ...
        {
            closest = findClosestInVector(epochLocations, newEpochIdx, 1, epochLocations.size()-1);
            int P0 = (epochLocations[closest + 1] - epochLocations[closest - 1])/2;
            int win_size = 2*P0+1;

            int curEpoch = epochLocations[closest];

            std::vector<float> epoch;
            for(int i = 0; i < win_size; i++)
            {
                epoch.push_back(getHanningCoef(win_size, i)*bufferIn[curEpoch - P0 + i]);
            }
            overlapAddArray(&bufferOut[0], &epoch[0], newEpochIdx - P0, win_size);
            newEpochIdx += P1;
        }
        // ************************ END YOUR CODE HERE  ***************************** //
    }
